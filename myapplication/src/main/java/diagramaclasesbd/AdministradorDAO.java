	...
	public static boolean deleteAndDissociate(diagramaclasesbd.Administrador administrador)throws PersistentException {
		try {
			if (administrador.getUsuario() != null) {
				administrador.getUsuario().suscripciones.remove(administrador);
			}
			
			if (administrador.getEs_suscriptor() != null) {
				administrador.getEs_suscriptor().suscriptores.remove(administrador);
			}
			...
			diagramaclasesbd.Video[] lOtros_usuarios = administrador.otros_usuario.toArray();
